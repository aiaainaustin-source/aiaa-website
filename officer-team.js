/**
 * Officer Team: load officers.json and render flip cards (front: image, name, role; back: quote, contact).
 */
(function () {
  'use strict';
  ...
  function init() {
    var container = document.getElementById('officer-cards');
    if (!container) return;

    function onLoad(officers) {
      container.innerHTML = '';
      for (var i = 0; i < officers.length; i++) {
        container.appendChild(renderCard(officers[i], i));
      }
    }

    function onError() {
      container.innerHTML = '<p class="officer-cards-error">Unable to load officer profiles. Try again later.</p>';
    }

    var xhr = new XMLHttpRequest();
    // Cache-bust so updates to officers.json show up (browser was serving old 2-person copy)
    xhr.open('GET', 'officers.json?v=' + Date.now());
